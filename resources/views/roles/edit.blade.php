    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <div class="card-title">
                <h3 class="card-label">Editar Perfil
                    <div class="text-muted pt-2 font-size-sm">Edição de perfis de usuários do sistema</div>
                </h3>
            </div>
        ...
        {!! Form::open(['id' => 'basic-form', 'url' => ['roles', $role->id], 'method' => 'patch']) !!}
            <div class="card-body">
                @include('layout.mensagens')
                <div class="form-group row">
                    <div class="col-lg-3">
                        <label for="display_name">Perfil</label>
                        <input type="text" name="display_name" id="display_name" class="form-control" value="{{ $role->display_name }}" placeholder="Perfil"/>
                        <span class="form-text text-muted">Digite o nome do perfil</span>
                    </div>
                    <div class="col-lg-3">
                        <label for="name">Identificador</label>
                        <input type="text" name="name" id="name" class="form-control" value="{{ $role->name }}" placeholder="Chave"/>
                        <span class="form-text text-muted">Digite um identificador textual único</span>
                    </div>
                    <div class="col-lg-6">
                        <label for="description">Descrição</label>
                        <input type="text" name="description" id="description" class="form-control" value="{{ $role->description }}" placeholder="Descrição"/>
